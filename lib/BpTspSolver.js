...
(function() {
  ...
  var originalOnFatalErrorCallback = function(tsp, errMsg) { console.log("Request failed: " + errMsg); }
  ...
  function directions(mode) {
    ...
    if (numActive > maxTspSize) {
      console.log("Too many locations! You have " + numActive + ", but max limit is " + maxTspSize);
    } else {
      legsTmp = new Array();
      distances = new Array();
      durations = new Array();
  ...
  function addAddress(address, label, id, callback) {
	...
	if (status == google.maps.GeocoderStatus.OK) {
	  ...
	  if (results.length >= 1) {
	    ...
	    if (typeof callback == 'function')
	      callback(address, latLng, label, id);
	  }
	} else if (status == google.maps.GeocoderStatus.OVER_QUERY_LIMIT) {
	  setTimeout(function(){ addAddress(address, label, id, callback) }, 100); 
	} else {
	  --addressRequests;
  ...
  function BpTspSolver(map, panel, onFatalError) {
    if (tsp) {
      console.log('You can only create one BpTspSolver at a time.');
      return;
    }
  ...
  BpTspSolver.prototype.addAddressWithLabel = function(address,  label, id, callback) {
    ++addressRequests;
    ++requestNum;
    tsp.addAddressAgain(address, label, id, callback, requestNum - 1);	
  }

  BpTspSolver.prototype.addAddress = function(address, callback) {
    tsp.addAddressWithLabel(address, null, null, callback);
  };

  BpTspSolver.prototype.addAddressAgain = function(address, label, id, callback, queueNum) {
    if (addressProcessing || queueNum > currQueueNum) {
      setTimeout(function(){ tsp.addAddressAgain(address, label, id, callback, queueNum) }, 100);
      return;
    }
    addAddress(address, label, id, callback);
  };
  ...
  BpTspSolver.prototype.solveRoundTrip = function(callback) {
    if (doNotContinue) {
      console.log('Cannot continue after fatal errors.');
      return;
    }
